...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        # Probability of the gene for this person
        if mother is None and father is None:
            # No parental info: use unconditional probability
            p_gene = PROBS["gene"][genes]
        else:
            # With parents: compute based on inheritance and mutation
            passes = {}
            for parent in [mother, father]:
                if parent in one_gene:
                    passes[parent] = 0.5
                elif parent in two_genes:
                    passes[parent] = 1 - PROBS["mutation"]
                else:
                    passes[parent] = PROBS["mutation"]

            if genes == 2:
                p_gene = passes[mother] * passes[father]
            elif genes == 1:
                p_gene = passes[mother] * (1 - passes[father]) + (1 - passes[mother]) * passes[father]
            else:  # genes == 0
                p_gene = (1 - passes[mother]) * (1 - passes[father])
